...
# PSRO related
flags.DEFINE_string(
    "meta_strategy_method", "alpharank", "Name of meta strategy computation method."
)
...
flags.DEFINE_bool(
    "symmetric_game",
    False,
    "Whether to consider the current " "game as a symmetric game.",
)
...
flags.DEFINE_string(
    "oracle_type",
    "BR",
    "Choices are DQN, PG (Policy " "Gradient) or BR (exact Best Response)",
)
flags.DEFINE_integer(
    "number_training_episodes",
    int(1e4),
    "Number training " "episodes per RL policy. Used for PG and DQN",
)
...
flags.DEFINE_integer(
    "update_target_network_every", 1000, "Update target " "network every [X] steps"
)
...
def print_policy_analysis(policies, game, step, verbose=False):
    ...
    if verbose:
        ...
        for player, cur_set in enumerate(unique_policies):
            ...
            wandb.log({f"P{player} # unique policies": len(cur_set)}, step=step)
    print("")
...
def gpsro_looper(env, oracle, agents):
    ...
    training_strategy_selector = (
        FLAGS.training_strategy_selector or strategy_selectors.probabilistic
    )
    ...
    for gpsro_iteration in range(FLAGS.gpsro_iterations):
        ...
        if FLAGS.verbose:
            ...
            # log meta game as heatmap

            # log meta strategy as bar chart
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 4))
            plt.title("Meta Strategies")
            ax1.bar(range(len(meta_probabilities[0])), meta_probabilities[0])
            ax2.bar(range(len(meta_probabilities[1])), meta_probabilities[1])
            ax1.set_xlabel("P0")
            ax2.set_xlabel("P1")
            plt.tight_layout()
            wandb.log({"Meta Strategies": fig}, step=gpsro_iteration)
        ...
        if env.game.get_type().dynamics == pyspiel.GameType.Dynamics.SEQUENTIAL:
            ...
            _ = print_policy_analysis(policies, env.game, gpsro_iteration, FLAGS.verbose)
            if FLAGS.verbose:
                ...
                wandb.log(
                    {
                        "Exp": exploitabilities / 2,
                        "Exp P0": expl_per_player[0],
                        "Exp P1": expl_per_player[1],
                    },
                    step=gpsro_iteration,
                )
...
def main(argv):
    ...
    game = pyspiel.load_game_as_turn_based(
        FLAGS.game_name, {"players": FLAGS.n_players}
    )
    ...
    run = wandb.init(
        config=flags_dict,
        project="evoPSRO",
        tags=["baseline:leduc_dqn_psro", "open_spiel"],
    )
